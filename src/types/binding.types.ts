/**
 * Ссылка на свойство ДРУГОГО компонента сцены (или своего же) — расширяет скоуп
 * биндинга помимо собственных свойств-тегов. Имя `varName` становится переменной
 * в коде (как имя свойства-тега), но значение в рантайме приходит не по `tag_id`,
 * а по `propertyId` в `properties[]` UPDATE (записи серверных Java-скриптов).
 *
 * Снимки `componentLabel`/`propertyName`/`valueType` — только для UI; ключ
 * маршрутизации в рантайме — `propertyId` (= серверный `PropertyCreateDto.id`).
 */
export interface PropertyRef {
  /** Имя переменной в коде (по умолчанию = имя свойства; при коллизии — псевдоним). */
  varName: string;
  /** Серверный id свойства-источника → ключ маршрутизации properties[] в рантайме. */
  propertyId: number;
  /** key компонента-источника на сцене (для отображения/переоткрытия модалки). */
  componentKey: string;
  /** Серверный id компонента-источника (стабилен между загрузками сцены). */
  componentId: number | null;
  /** Снимок названия компонента-источника для UI. */
  componentLabel: string;
  /** Снимок имени свойства для UI. */
  propertyName: string;
  /** Снимок типа значения (string/integer/float/boolean/date). */
  valueType?: string;
}

/**
 * Поле свойства, подставляемое в ячейку таблицы.
 *
 * `value` — ЖИВОЕ значение, единственное, что идёт через рантайм. Остальные поля
 * статические: лежат в самом `element.properties` и подставляются рендером одинаково
 * в редакторе и в мониторе.
 */
export type CellSourceField =
  | "value"
  | "name"
  | "tag_id"
  | "value_type"
  | "property_type"
  | "default_value"
  | "description";

export interface CellBinding {
  row: number;
  col: number;
  /** Имя свойства ЭТОЙ таблицы. */
  propertyName: string;
  field: CellSourceField;
}

/**
 * Биндинг — JavaScript-скрипт, исполняемый НА КЛИЕНТЕ движком режима монитора
 * (в отличие от `element.scripts` — Java-скриптов, исполняемых на сервере).
 *
 * Скоуп кода: свойства-теги элемента (`properties` с `property_type === "Тег"`)
 * доступны по ИМЕНИ свойства как объекты `{V, RAW}` (`V` — число, если значение
 * парсится, иначе строка). Плюс ссылки на свойства других компонентов сцены
 * (`propertyRefs`, доступны так же по имени переменной). Плюс API движка:
 * `setState("Имя состояния")`, `setProp("color", "#f00")`, `self` (rendered-снимок
 * элемента, только чтение).
 *
 * Пример: `if (LINE1FQT1 > 100) { setState("Авария") } else { setState("Нормальное") }`
 *
 * Сериализация: биндинг целиком уезжает JSON-строкой в DTO-поле `script`
 * (бэкенд хранит его как опак-строку — контракт не меняется; поэтому `propertyRefs`
 * round-trip'ится без правок контракта).
 */
export interface TagBinding {
  /** Маркер схемы — по нему parseBindings отличает «наши» биндинги от легаси-мусора. */
  v: 1;
  /** createUuid() — клиентский id, живёт внутри JSON. */
  id: string;
  name: string;
  enabled: boolean;
  /** JavaScript-исходник; компилируется один раз на сцену через new Function. */
  code: string;
  /**
   * tag_id-ы, изменение которых запускает биндинг. Не задано — биндинг
   * запускается на любой тег из своего скоупа (свойств-тегов элемента).
   */
  triggers?: string[];
  /**
   * Ссылки на свойства других компонентов сцены. Не задано / пусто —
   * скоуп биндинга состоит только из свойств-тегов самого элемента (легаси).
   */
  propertyRefs?: PropertyRef[];
  /**
   * Прямая привязка «значение элемента ← значение свойства» без кода/имени:
   * `code` сгенерирован автоматически (`setProp(target, <ref>)`), UI не
   * показывает редактор кода. Внутри — обычный биндинг (движок не меняется).
   */
  direct?: boolean;
  /**
   * Ключ рендер-пропа/рантайм-оверрайда, в который пишет прямая привязка (direct: true):
   * `"value"` — элемент целиком (см. BindingsTab), `"cell_${row}_${col}"` — конкретная
   * ячейка таблицы. Используется UI для обнаружения «эта цель уже привязана» без
   * парсинга `code`.
   */
  directTarget?: string;
  /**
   * Путь ТЕГА для прямой привязки «значение элемента ← значение тега».
   *
   * Отличие от `propertyRefs`: там источник — свойство другого компонента, и значение
   * приходит номером свойства в `properties[]` кадра. Здесь источник — сам тег, и
   * собственных свойств элементу не нужно вовсе: рантайм пишет значение в `directTarget`
   * напрямую по `tag_id`, минуя JS (`code` у такой привязки пуст).
   *
   * Ровно так прогресс-бар получает значение: одна привязка к тегу — и он следует за ним.
   */
  tag?: string;
  /**
   * Привязка ЯЧЕЙКИ таблицы к полю собственного свойства этой же таблицы.
   *
   * Свойства висят на таблице целиком (`element.properties`), а здесь описано, какое
   * поле какого свойства попадает в какую ячейку. Пришло на смену прежней жёсткой
   * конвенции «строка = свойство с position, колонка 0 — номер, 1 — имя, 2 — значение»:
   * раскладку задаёт пользователь, а не рендер.
   *
   * Свойство адресуется ИМЕНЕМ: `propertyId` нестабилен между пересохранениями
   * таблицы (см. RuntimePropertyUpdate), по имени же ходят рецепты и WS-кадр.
   *
   * Не путать с `direct`/`directTarget` — та привязка про значение свойства ДРУГОГО
   * компонента и работает своим путём.
   */
  cell?: CellBinding;
  /**
   * Серверный id биндинга — только у пришедших из бэкенда. То же обязательство, что у
   * `ComponentState.serverId`: пришло с `id` — вернуть с тем же, иначе переименование
   * биндинга читается сервером как «удалили один и создали другой».
   *
   * В JSON внутри `script` НЕ попадает: `encodeBindings` вырезает его перед
   * сериализацией, потому что серверные поля живут в DTO-обёртке, а не в опак-строке
   * (иначе после восстановления версии мы вернули бы устаревший id из старого JSON).
   */
  serverId?: number | string;
  /**
   * Пара «свойство, к которому привязан биндинг», как её отдал сервер. С 14.08.2026
   * биндинг приходит с обоими полями, и контракт требует вернуть оба как получили: имя
   * там не для чтения — ответ сохранения служит снимком версии, а номер свойства снимок
   * не переживает (свойство, удалённое и заведённое заново, получает новый id).
   * Не задано — биндинг новый, `component_property_id` вычисляем сами.
   */
  componentPropertyId?: number;
  componentPropertyName?: string;
}

/**
 * Формат биндинга в ComponentCreateDto.
 *
 * `id` и `component_property_name` присылаем только тогда, когда сервер их прислал нам
 * (§2 контракта версий). Сам биндинг едет JSON-строкой в `script` — опак-поле, которое
 * бэкенд не разбирает.
 */
export type BindingDto = {
  id?: number | string;
  /**
   * Необязательное: у элемента может не быть НИ ОДНОГО сохранённого свойства — так
   * приезжает только что поставленный шаблон, где свойства ещё черновики. Раньше в
   * этом случае слался `0`, но нулевого свойства не существует; «поля нет» — это
   * задокументированное «биндинг новый».
   */
  component_property_id?: number;
  component_property_name?: string;
  name: string;
  script: string;
};

/**
 * Обработчик события элемента (onClick/onDoubleClick) — JavaScript, исполняемый
 * НА КЛИЕНТЕ в режиме монитора при клике/двойном клике по элементу.
 *
 * Скоуп: как у биндинга (свойства-теги + `propertyRefs` доступны по имени как
 * `{V, RAW}`), плюс API записи: `setProperty("Имя", значение)` — пишет значение
 * в свойство (по propertyRefs), на него реагируют биндинги других элементов;
 * `setProp("key", v)` / `setState("Имя")` — визуал/состояние самого элемента;
 * `self` — rendered-снимок.
 *
 * Пример: `setProperty("Test", Test + 1)` — инкремент свойства объекта.
 */
export interface ElementEventHandler {
  /** JavaScript-исходник обработчика. */
  code: string;
  /** Свойства объектов сцены в скоупе (чтение `Имя` + запись `setProperty`). */
  propertyRefs?: PropertyRef[];
}

/** Имена поддерживаемых событий. */
export type ElementEventName = "onClick" | "onDoubleClick";

/** Запись обработчика события в массиве `element.events` (бэкенд хранит events как массив узла). */
export interface ElementEventEntry {
  event_type: ElementEventName;
  handler: ElementEventHandler;
  /**
   * Серверный id события — только у пришедших из бэкенда. Сами события сервер сопоставляет
   * по `event_type`, и переименовать их нельзя, так что ложного конфликта здесь не бывает;
   * id возвращаем ради общего правила «пришло с id — вернуть с тем же» и чтобы снимок
   * версии оставался точным.
   */
  serverId?: number | string;
}

/** События элемента, обрабатываемые на клиенте в режиме монитора. */
export type ElementEvents = ElementEventEntry[];
